...
from numpy import (sin, cos, tan, log, log10, pi, average,
                   sqrt, std, deg2rad, rad2deg, linspace, asarray, arange)
...
trial_times = list(arange(0.025, 0.525, 0.025))
stim_path = './stims/outputs/stim_{f}_{n}.png'
...
text_4 = visual.TextStim(win=win, name='text_4',
    text='Now that you have some practice with the task, we will move in to the full experiment, consisting of 50 trials.\n\nPress the space bar when you are ready to continue.',
    font='Open Sans',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
image = visual.ImageStim(
    ...
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
    texRes=200.0, interpolate=True, depth=0.0)
...
while continueRoutine:
    ...
    if text.status == STARTED:
        if bool(event.getKeys(keyList = 'space')):
...
for thisTrial_2 in trials_2:
    ...
    # update component parameters for each repeat
    # keep track of which components have finished
    prac_respComponents = [text_5]
    for thisComponent in prac_respComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    prac_respClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "prac_resp"-------
    event.clearEvents()
    while continueRoutine:
        # get current time
        t = prac_respClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=prac_respClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
...
for thisTrial in trials:
    ...
    continueRoutine = True
    rand_select = randint(0, len(trial_times))
    rand_time = trial_times[rand_select]
    ...
    #add data
    thisExp.addData('stim_interval', rand_time)
    thisExp.addData('correct_resp', fig + 1)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if image.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > image.tStartRefresh + rand_time-frameTolerance:
                # keep track of stop time/frame for later
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    respComponents = [key_resp, text_2]
